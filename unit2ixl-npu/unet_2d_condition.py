...
class UNet2DConditionModel(
    ...
    def forward(
        ...
        if_skip: bool = False,
        inputCache: torch.FloatTensor = None,
        ...
        for tmp, downsample_block in enumerate(self.down_blocks):
            if tmp >=2 and if_skip:
                break
        ...
        if not if_skip:
            for i, upsample_block in enumerate(self.up_blocks):
                ...
                if use_cache and (not if_skip) and (i == 0):
                    inputCache = sample
            ...
            for i, upsample_block in enumerate(self.up_blocks):
                is_final_block = i == len(self.up_blocks) - 1
                if not is_final_block:
                    upsample_size = down_block_res_samples[0].shape[2:]
        ...
        if not use_cache or if_skip:
            return UNet2DConditionOutput(sample=sample)

        return (sample, inputCache)
